<script lang="ts">
  ...
  function prevImage() {
    currentIndex = (currentIndex - 1 + show.images.length) % show.images.length;
  }

  function nextImage() {
    currentIndex = (currentIndex + 1) % show.images.length;
  }
...
<div class="relative w-full h-64 md:h-80 lg:h-96 z-10">
  {#if show.backdrop}
    <div
      ...
      style="background-image: url('{show.backdrop}')"
    ></div>
  {/if}

  <!-- Back Arrow -->
  <a href="/" aria-label="Go back to shows list" class="absolute top-4 left-4 md:top-6 md:left-6 bg-black/50 rounded-full p-2 hover:bg-black/70 transition z-20">
...
<!-- Tabs -->
    <div class="bg-gray-900 flex gap-4 mb-6 border-b border-gray-700 relative py-2">
      {#each ["Details", "Episodes", "Screenshots", "Videos"] as tab, i (tab)}
        <button
          bind:this={tabRefs[i]}
          ...
          on:click={() => setActive(tab as typeof activeTab, i)}
        >
          {tab}
        </button>
      {/each}
    ...
    <div class="mt-2"></div>
      ...
      <div class="bg-gray-800 p-6 md:p-8 rounded-lg shadow-lg space-y-4 animate-fadeIn">
        <h1 class="text-2xl md:text-3xl font-bold">{show.title} <span class="text-gray-400 text-lg md:text-xl">({show.year ?? "-"})</span></h1>
        <p class="text-gray-300">{show.overview ?? "-"}</p>
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
        <p><strong>Genres:</strong> {show.genres?.join(", ") ?? "-"}</p>
        <p><strong>Rating:</strong> {show.rating ?? "-"} ({show.rating_count ?? 0} votes)</p>
        ...
        <p><strong>Adult:</strong> {show.adult ? "Yes" : "No"}</p>
        <p><strong>IMDb:</strong> {show.imdb_id ?? "-"}</p>
        <p><strong>TMDb:</strong> {show.tmdb_id ?? "-"}</p>
        </div>
      </div>
      {/if}
      ...
      {#if activeTab === "Episodes"}
        <div class="space-y-6 animate-fadeIn">
          ...
          {#if show.episodes_list?.length > 0}

            {#each Object.entries(groupBySeason(show.episodes_list)) as [season, eps]}
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        {#each show.images.slice(0,12) as img, i}
          <button
            type="button"
            ...
            aria-label={`Open screenshot ${i + 1} from ${show.title} in lightbox`}
            on:click={() => openLightbox(i)}
            on:keydown={(e) => { if (e.key === "Enter" || e.key === " ") openLightbox(i); }}
          >
            <img
              src={img}
              alt={`Screenshot from ${show.title}`}
              class="rounded-lg"
            />
          </button>
        {/each}
      ...
      {#if showLightbox}
        <div class="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50">
          ...
          <img src={show.images[currentIndex]} alt="Screenshot" class="max-h-[90vh] max-w-[90vw] rounded-lg shadow-lg" />
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-8">
        ...
        {#each show.videos.slice(0,6) as vid}
