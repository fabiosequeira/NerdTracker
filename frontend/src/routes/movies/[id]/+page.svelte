<script lang="ts">
  ...
  let movie: any = null;
  let activeTab: "Details" | "Screenshots" | "Videos" = "Details";
  ...
  function prevImage() {
    currentIndex = (currentIndex - 1 + movie.images.length) % movie.images.length;
  }

  function nextImage() {
    currentIndex = (currentIndex + 1) % movie.images.length;
  }
...
<div class="relative w-full h-64 md:h-80 lg:h-96 z-10">
  ...
  <!-- Back Arrow -->
  <a href="/" aria-label="Go back to movies list" class="absolute top-4 left-4 md:top-6 md:left-6 bg-black/50 rounded-full p-2 hover:bg-black/70 transition z-20">
...
<!-- Tabs -->
    ...
    <div class="mt-2"></div>
      ...
      <div class="bg-gray-800 p-6 md:p-8 rounded-lg shadow-lg space-y-4 animate-fadeIn">
        <h1 class="text-2xl md:text-3xl font-bold">{movie.title} <span class="text-gray-400 text-lg md:text-xl">({movie.year ?? "-"})</span></h1>
        <p class="text-gray-300">{movie.overview ?? "-"}</p>
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
        <p><strong>Genres:</strong> {movie.genres?.join(", ") ?? "-"}</p>
        <p><strong>Rating:</strong> {movie.rating ?? "-"} ({movie.rating_count ?? 0} votes)</p>
        <p><strong>Runtime:</strong> {movie.runtime ?? "-"} min</p>
        <p><strong>Popularity:</strong> {movie.popularity ?? "-"}</p>
        <p><strong>Adult:</strong> {movie.adult ? "Yes" : "No"}</p>
        <p><strong>IMDb:</strong> {movie.imdb_id ?? "-"}</p>
        <p><strong>TMDb:</strong> {movie.tmdb_id ?? "-"}</p>
        </div>
      </div>
      {/if}
      ...
      {#if showLightbox}
        <div class="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50">
          ...
          <img src={movie.images[currentIndex]} alt="Screenshot" class="max-h-[90vh] max-w-[90vw] rounded-lg shadow-lg" />
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-8">
        ...
        {#each movie.videos.slice(0,6) as vid}
